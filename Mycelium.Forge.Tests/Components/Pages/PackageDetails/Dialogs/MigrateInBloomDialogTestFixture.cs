// ------------------------------------------------------------------------------------------------
// <copyright file="MigrateInBloomDialogTestFixture.cs" company="Starion Group S.A.">
// 
...
namespace Mycelium.Forge.Tests.Components.Pages.PackageDetails.Dialogs
{
    ...
    public class MigrateInBloomDialogTestFixture
    {
        ...
        [Test]
        public async Task VerifyOnOpenInBloomClicked()
        {
            MigrateInBloomResult? capturedResult = null;
            var onResult = new EventCallbackFactory().Create(this, (MigrateInBloomResult r) => { capturedResult = r; });
            ...
            dialog.Instance.OnVersionConstraintChanged(string.Empty);
            await dialog.InvokeAsync(() => dialog.Instance.OnOpenInBloomClicked());
        ...
        {
            var dialog = this.context.Render<MigrateInBloomDialog>(parameters => { parameters.AddCascadingValue(this.dialogReferenceMock.Object); });

            dialog.Instance.OnProjectChanged("Ground Station Network");
            ...
            using (Assert.EnterMultipleScope())
            {
                Assert.That(withValue, Is.EqualTo("Ground Station Network"));
