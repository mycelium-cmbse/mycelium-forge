...
namespace Mycelium.Forge.Tests.Components.Pages
{
    ...
    public class LoginTestFixture
    {
        ...
        public async Task VerifyOnContinueWithSso()
        {
            ...
            this.viewModelMock.Setup(x => x.ContinueWithSso()).Returns(Result.Ok());
            await loginPage.InvokeAsync(() => ssoButton.ClickAsync());

            using (Assert.EnterMultipleScope())
            {
                this.viewModelMock.Verify(x => x.ContinueWithSso(), Times.Once);
                Assert.That(this.toastService.Toasts, Has.Count.EqualTo(1));
            }

            this.viewModelMock.Setup(x => x.ContinueWithSso()).Returns(Result.Fail("SSO provider unavailable"));
            await loginPage.InvokeAsync(() => ssoButton.ClickAsync());

            using (Assert.EnterMultipleScope())
            {
                this.viewModelMock.Verify(x => x.ContinueWithSso(), Times.Exactly(2));
