...
namespace Mycelium.Forge.Serializer.Json
{
    ...
    internal static class PackageTypeDeSerializer
    {
        ...
        internal static IPackageType DeSerialize(ref Utf8JsonReader reader, ILoggerFactory loggerFactory = null)
        {
            ...
            while (reader.Read() && reader.TokenType != JsonTokenType.EndObject)
            {
                ...
                if (reader.ValueTextEquals("owner"u8))
                {
                    ...
                    if (reader.TokenType == JsonTokenType.Null)
                    {
                        ...
                        logger.LogDebug($"the PackageType.Owner property was not found in the Json. The value is set to Guid.Empty");
                    }
                    else
                    {
            ...
            if (!ownerSeen)
            {
                logger.LogDebug("the owner Json property was not found in the PackageType: {Id}", dtoInstance.Id);
            }
